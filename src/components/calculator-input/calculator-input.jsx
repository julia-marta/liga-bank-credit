...
const CalculatorInput = ({label, name, suffix, stringValue, value, isError, isErrorMessage, isControls, onChangeInput, onBlurInput, onClickControl}) => {
    return (
        <>
          ...
          <div className={`form__input form__input--wrapper ${isError ? `form__input--error` : ``}`} tabIndex="0" >
            {isControls && <button id="decrease" className="form__сontrol form__сontrol--decrease" type="button"
            aria-label="Уменьшить" onClick={onClickControl} />}
            ...
            {isControls && <button id="increase" className="form__сontrol form__сontrol--increase" type="button"
            aria-label="Увеличить" onClick={onClickControl} />}
